...
# 🌐 SMARTLING MCP ULTRA - COMPLETE API INSTALLER (FIXED VERSION)
# Clone repo + Complete API installation in one command
# Version: 4.0.1 - Complete API Coverage + CURSOR CONFIGURATION BUG FIXES
#
# FIXES APPLIED:
# ✅ Fixed: Cursor config file path (.cursor/mcp.json instead of .cursor/settings.json)
# ✅ Fixed: Cursor config format ("mcpServers" instead of "mcp.servers")
# ✅ Fixed: Uses mcp-complete-api.js for all 29 Smartling API tools
# ✅ Fixed: Consistent naming (smartling-complete instead of smartling-ultra)
...
show_success_message() {
    ...
    if [[ "$INSTALL_FOR_CLAUDE" == true ]]; then
        ...
        echo -e "${CYAN}   2. ✨ The 'smartling-complete' server will be available automatically${NC}"
