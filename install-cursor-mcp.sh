...
if [[ -f "$CURSOR_CONFIG_FILE" && -s "$CURSOR_CONFIG_FILE" ]] && command -v jq > /dev/null 2>&1; then
    echo "🔍 Checking for existing Smartling credentials..."
    EXISTING_USER_ID=$(jq -r '.mcpServers.smartling.env.SMARTLING_USER_IDENTIFIER // ""' "$CURSOR_CONFIG_FILE" 2>/dev/null)
    EXISTING_USER_SECRET=$(jq -r '.mcpServers.smartling.env.SMARTLING_USER_SECRET // ""' "$CURSOR_CONFIG_FILE" 2>/dev/null)
    EXISTING_ACCOUNT_UID=$(jq -r '.mcpServers.smartling.env.SMARTLING_ACCOUNT_UID // ""' "$CURSOR_CONFIG_FILE" 2>/dev/null)
fi
