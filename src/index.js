// ✨ Smartling MCP Server - Complete translation server for Claude & Cursor ✨
...
const sendJSON = (res, status, data) => {
  if (typeof res.status === 'function') {
    ...
    // Native Node.js HTTP style (Local server)
...
const SmartlingTools = {
  ...
  // === STRING SEARCH TOOLS ===
  async smartling_search_strings(args, client) {
    const { projectId, searchText, localeId, fileUris, includeTimestamps = true, limit = 50 } = args;
    if (!projectId || !searchText) throw new Error('projectId and searchText are required');

    const params = new URLSearchParams();
    params.append('q', searchText);
    if (localeId) params.append('localeId', localeId);
    if (fileUris) fileUris.forEach(uri => params.append('fileUri', uri));
    if (limit) params.append('limit', limit.toString());
    if (includeTimestamps) params.append('includeTimestamps', 'true');

    const response = await client.makeRequest(`/strings-api/v2/projects/${projectId}/strings/search?${params.toString()}`, 'GET');
    return {
      content: [{
        type: "text",
        text: `🔍 **String Search Results:**\n\nFound ${response.data.length} strings matching "${searchText}"\n\n${JSON.stringify(response.data, null, 2)}`
      }]
    };
  },

  async smartling_get_string_details(args, client) {
    const { projectId, hashcode, localeId } = args;
    if (!projectId || !hashcode || !localeId) throw new Error('projectId, hashcode, and localeId are required');

    const response = await client.makeRequest(`/strings-api/v2/projects/${projectId}/locales/${localeId}/strings/${hashcode}`, 'GET');
    return {
      content: [{
        type: "text",
        text: `📄 **String Details:**\n\n${JSON.stringify(response.data, null, 2)}`
  ...
  async smartling_get_recently_localized(args, client) {
    const { projectId, localeId, limit = 20, fileUris } = args;
    if (!projectId || !localeId) throw new Error('projectId and localeId are required');
    ...
    const response = await client.makeRequest(`/strings-api/v2/projects/${projectId}/strings?${params.toString()}`, 'GET');
    return {
      content: [{
        type: "text",
        text: `⏰ **Recently Localized Strings:**\n\nLast ${limit} modified strings:\n\n${JSON.stringify(response.data, null, 2)}`
      }]
    };
  },
...
const toolDefinitions = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        searchText: { type: "string", description: "Text to search for in strings" },
        localeId: { type: "string", description: "Optional: specific locale to search in" },
        fileUris: { type: "array", items: { type: "string" }, description: "Optional: limit search to specific files" },
        includeTimestamps: { type: "boolean", description: "Include creation/modification timestamps", default: true },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        projectId: { type: "string", description: "The project ID" },
        hashcode: { type: "string", description: "The string hashcode identifier" },
        localeId: { type: "string", description: "The locale ID" }
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        projectId: { type: "string", description: "The project ID" },
        localeId: { type: "string", description: "The locale ID" },
        limit: { type: "number", description: "Number of recent results to return", default: 20 },
        fileUris: { type: "array", items: { type: "string" }, description: "Optional: limit to specific files" }
      },
